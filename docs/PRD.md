...
| S-02b | **Itinerary Agent** | 扩展 POI 池并按天分配上午/下午/晚间 | P0 |
...
**原则：仅使用用户勾选的景点**，不自动加入未选择的同城 POI。单个景点可跨多个半天/多天（例如 6/6–6/7 上午古城、6/7 下午–6/8 洱海）。

| 步骤 | 说明 |
|------|------|
| Step 1 | 用户多选意向景点（顺序有意义） |
| Step 2 | 生成 POI 池 = 用户所选列表（去重，保序） |
| Step 3 | 将 `天数 × 2` 个半天时段（上午/下午）按顺序切成连续块 |
| Step 4 | 依次分配给各景点，每块内为同一 POI |

手动安排模式：用户自行指定每日景点列表，系统按顺序映射为上午 / 下午 / 晚间时段。
